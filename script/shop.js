const mans = [
    {
    "id": 1,
    "name": "Ящик дерево #01",
    "price": "120",
    "photo": "./media/compositions/mans/IMG_20250811_154329_028.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_154332_716.jpg",
    "description": "Перец, помидоры черри, мясные чипсы, сыр косичка, колбасы высших сортов 3 вида, пастрома, зелень<br>Ящик дерево<br><br>Заказ 3-5 дней"
  },
  {
    "id": 2,
    "name": "Конверт #02",
    "price": "100",
    "photo": "./media/compositions/mans/IMG_20250811_154709_814.jpg",
    "photo2": "",
    "description": "Помидоры черри, мясные чипсы, колбасы высших сортов 3 вида, сыр косичка, сыр шарики, зелень. <br> Напиток в стоимость не входит<br><br>Заказ 3-5 дней"
  },
  {
    "id": 3,
    "name": "Букет #03",
    "price": "70",
    "photo": "./media/compositions/mans/IMG_20250811_154812_304.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_154824_546.jpg",
    "description": "Орехи в панировке, арахис соленый, сухарики, колбасы высших сортов 3 видов, сыр косичка, мясные чипсы.Напиток в стоимость не входит. Цвет упаковки можно обсудить под запрос.<br><br>Заказ 3-5 дней"
  },
  {
    "id": 4,
    "name": "Бокс #04",
    "price": "60",
    "photo": "./media/compositions/mans/IMG_20250811_155451_426.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_155447_729.jpg",
    "description": "Микс орехов, мясные чипсы, колбаски разные, сухарики. <br>Напиток в стоимость не входит. <br> Размер бокса 20*20<br><br>Заказ 3-5 дней"
  },
  {
    "id": 5,
    "name": "Ящик дерево #05",
    "price": "160",
    "photo": "./media/compositions/mans/IMG_20250811_160041_778.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_160039_725.jpg",
    "description": "Перец Чили, помидоры черри, мясные чипсы, колбасы высших сортов 3-4 вида, паструма, сыр косичка, розмарин.<br> Ящик дерево.<br> Напиток в стоимость не входит <br><br>Заказ 3-5 дней"
  },
  {
    "id": 6,
    "name": "Ящик дерево #06",
    "price": "100",
    "photo": "./media/compositions/mans/IMG_20250811_155706_925.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_155708_876.jpg",
    "description": "Помидоры черри, сыр косичка, сыр палочки, колбаска божале в красном вине, мясные чипсы, колбасы высших сортов 2 видов.<br> Ящик дерево.<br> Напиток в стоимость не входит<br><br>Заказ 3-5 дней "
  },
  {
    "id": 7,
    "name": "Ящик дерево #07",
    "price": "140",
    "photo": "./media/compositions/mans/IMG_20250811_155609_021.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_155606_992.jpg",
    "photo3": "./media/compositions/mans/IMG_20250811_154926_700.jpg",
    "description": "Помидоры черри, колбасы высших сортов 3-4 вида, сыр косичка, паструма, мясные чипсы, зелень.<br> Ящик дерево.<br> Напиток в стоимость не входит <br><br>Заказ 3-5 дней"
  },
  {
    "id": 8,
    "name": "Конверт Мини #08",
    "price": "60",
    "photo": "./media/compositions/mans/IMG_20250811_155331_879.jpg",
    "photo2": "./media/compositions/mans/IMG_20250811_155328_054.jpg",
    "description": "Орехи в панировке, сыр косичка, помидоры черри, мясные чипсы, колбаски 3 видов, зелень. <br> Напиток в стоимость не входит. <br><br>Заказ 3-5 дней"
  },
  {
    "id": 9,
    "name": "Бокс дерево #09",
    "price": "65",
    "photo": "./media/compositions/mans/IMG_20250811_155127_578.jpg",
    "description": "Мясные чипсы, сухарики, орехи в панировке, рыбка жёлтый полосатик, колбаски 2 видов.<br> Бокс дерево размеры 25*25. <br>Напиток в стоимость не входит.<br><br>Заказ 3-5 дней "
  },
  {
    "id": 10,
    "name": "Букет круглый #10",
    "price": "160",
    "photo": "./media/compositions/mans/IMG_20250811_155111_220.jpg",
    "description": "Перец, помидоры черри, хлебушек, мяные чипсы, сыр косичка, колбасы 3-4 вида, сьедобная зелень. <br><br> Сьедобный диаметр 35-40см. <br><br>Заказ 3-5 дней "
  },
  {
    "id": 11,
    "name": "Ящик дерево 'Премиум' #11",
    "price": "200",
    "photo": "./media/compositions/mans/IMG_20250811_155036_754.jpg",
    "description": "Сыр палочки, колбаса Божоле, паструма, Парма с плесенью, кабандосы классик, мясные чипсы, сьедобная зелень. Форель, вомер, жёлтый полосатик, снежки лосось, палочки минтай. <br>Ящик дерево.<br> Напиток в стоимость не входит <br><br>Заказ 3-5 дней"
  },
  {
    "id": 12,
    "name": "Ящик сырный #12",
    "price": "130",
    "photo": "./media/compositions/mans/12_01.jpg",
    "photo2": "./media/compositions/mans/12_02.jpg",
    "description": "Орехи в панировке 4 шт, фисташки 1шт, мясные чипсы, колбаска с плесенью, 4 вида сыра - косичка, шарики, Джил, палочки, инжир ( по сезону), зелень. <br><br> Ящик дерево. <br><br> Заказ 3-5 дней"
  },
  {
    "id": 13,
    "name": "Мужской букет #13",
    "price": "85",
    "photo": "./media/compositions/mans/13_01.jpg",
    "photo2": "./media/compositions/mans/13_02.jpg",
    "description": "Паструма, колбаска с плесенью, помидоры черри, мясные чипсы, колбаски, зелень <br>Напиток в стоимость не входит <br><br>Заказ 3-5 дней"
  },
  {
    "id": 14,
    "name": "Мужской букет #14",
    "price": "80",
    "photo": "./media/compositions/mans/1401.jpg",
    "photo2": "./media/compositions/mans/1402.jpg",
    "description": "Орехи в панировке, соленый арахис, сухарики, сыр косичка, колбаски  <br>Напиток в стоимость не входит <br><br>Заказ 3-5 дней"
  },
  {
    "id": 15,
    "name": "Мужской букет #15",
    "price": "60",
    "photo": "./media/compositions/mans/15_01.jpg",
    "photo2": "./media/compositions/mans/15_02.jpg",
    "description": "Арахис в панировке, арахис соленый, сухарики, сыр косичка, чипсы мясные, колбаски  <br>Напиток в стоимость не входит <br><br>Заказ 2-3 дней"
  },
];

const womans = [
    {
    "id": 1,
    "name": "Композиция в шляпной коробке #01",
    "price": "150",
    "photo": "./media/compositions/woman/IMG_20250811_165419_941.jpg",
    "photo2": "./media/compositions/woman/IMG_20250811_165420_531.jpg",
    "description": "Роза <br>(возможна замена на другие цветы под ваш запрос)<br> Клубника + клубника в шоколаде. <br><br> Заказ 5-7 дней"
  },
  {
    "id": 2,
    "name": "Коробка круглая #02",
    "price": "180",
    "photo": "./media/compositions/woman/IMG_20250811_165420_835.jpg",
    "photo2": "./media/compositions/woman/IMG_20250811_165419_877.jpg",
    "description": "Цветы: роза, хризантемы <br>(возможна  замена на другие цветы под запрос)  <br><br> Размер 25*25*12 <br><br> Зефир с разными начинками, Рафаэлло, макаронс,  клубника/голубика в шоколаде (по наличию, по сезону) другие сладости. <br><br> Возможно наполнение под запрос. <br><br> Заказ 5-7 дней "
  },
  {
    "id": 3,
    "name": "Коробка сердце #03",
    "price": "150",
    "photo": "./media/compositions/woman/IMG_20250811_165420_448.jpg",
    "photo2": "./media/compositions/woman/IMG_20250811_165419_986.jpg",
    "description": "Цветы: хризантемы (возможна замена на другие цветы под запрос)  <br><br> Размер 23*20 <br><br> Королевские финики в белом и молочном шоколаде 10-12шт с начинками: сыр дор Блю, курага, различные виды орехов. <br><br>Заказ 5-7 дней "
  },
  {
    "id": 4,
    "name": "Коробка круглая #04",
    "price": "180",
    "photo": "./media/compositions/woman/IMG_20250811_165423_897.jpg",
    "photo2": "./media/compositions/woman/IMG_20250811_165423_888.jpg",
    "description": "Цветы: роза однаголовая, роза кустовая, зелень <br> (возможна замена на другие цветы под запрос) <br><br> Различные виды конфет, макаронс, различные виды орехов в шоколаде, марчмеллоу, сердца граната в белом шоколаде (по сезону, по наличию) <br><br> Возможно наполнение под запрос. <br><br> Заказ 5-7 дней "
  }
];

const child = [
    {
    "id": 1,
    "name": "Букет #01",
    "price": "60",
    "photo": "./media/compositions/child/IMG_20250811_165236_260.jpg",
    "photo2": "./media/compositions/child/IMG_20250811_165237_045.jpg",
    "photo3": "./media/compositions/child/IMG_20250811_165236_448.jpg",
    "description": "Шоколадные, вафельные батончики под запрос <br><br> Заказ 3-5 дней"
  },
  {
    "id": 2,
    "name": "Букет #02",
    "price": "150",
    "photo": "./media/compositions/child/IMG_20250811_165236_939.jpg",
    "photo2": "./media/compositions/child/IMG_20250811_165236_961.jpg",
    "description": "Разные виды зефира, мармелшоу, орехи в шоколаде, конфеты, другие сладости, пряники <br><br>Диаметр 35-40см <br><br>Заказ 3-5 дней "
  },
  {
    "id": 3,
    "name": "Букет #03",
    "price": "55",
    "photo": "./media/compositions/child/IMG_20250811_165236_726.jpg",
    "photo2": "./media/compositions/child/IMG_20250811_165236_340.jpg",
    "description": "Зефир, мармелшоу, макаронс, другие конфеты <br><br> Заказ 3-5 дней "
  },
  {
    "id": 4,
    "name": "Букет #04",
    "price": "120",
    "photo": "./media/compositions/child/51.jpg",
    "photo2": "./media/compositions/child/52.jpg",
    "description": "Напиток, мармелшоу, микс шоколадные/вафельные батончики, шоколад и другие сладости <br><br> Заказ 3-5 дней "
  }
];

const Fruits = [
  {
    "id": 1,
    "name": "Сумочка мини #01",
    "price": "43",
    "photo": "./media/compositions/oldFruits/IMG_20250811_163321_594.jpg",
    "photo2": "./media/compositions/oldFruits/IMG_2343.JPG",
    "photo3": "./media/compositions/oldFruits/IMG_20250811_163310_285.jpg",
    "description": "Курага, кумкват, финики,кизил, инжир, манго, маракуйя, макаронс, орехи пекан, макадами, миндаль. <br><br> Диаметр 15-16см <br><br>Заказ 3-5 дней "
  },
  {
    "id": 2,
    "name": "Сумочка средняя #02",
    "price": "53",
    "photo": "./media/compositions/oldFruits/IMG_2340.JPG",
    "photo2": "",
    "description": "Курага, кумкват, кизил, инжир, финики, манго, маракуйя, макаронс, орехи пекан, макадами, миндаль.<br><br> Диаметр 17-18см <br><br> Заказ 3-5 дней "
  },
  {
    "id": 3,
    "name": "Сумочка длинная #03",
    "price": "67",
    "photo": "./media/compositions/oldFruits/IMG_2341.JPG",
    "photo2": "./media/compositions/oldFruits/IMG_2342.JPG",
    "description": "Курага, кумкват, кизил, инжир, финики, манго, маракуйя, макаронс, орехи пекан, макадами, миндаль, орехи в белом шоколаде, манго кубик. <br><br>Размеры сьедобной части 25*15/16 <br><br>Заказ 3-5 дней "
  },
  {
    "id": 4,
    "name": "Сумочка длинная #04",
    "price": "105",
    "photo": "./media/compositions/oldFruits/IMG_20250811_163419_162.jpg",
    "photo2": "./media/compositions/oldFruits/IMG_20250811_163423_905.jpg",
    "description": "Цветы: цимбидиум, хризантема, зелень <br><br> Курага, кумкват, кизил, инжир, финики, манго, маракуйя, макаронс, орех макадами, миндаль, орехи в белом шоколаде, манго кубик, ананас, груша/хурма. <br><br> Заказ 3-5 дней "
  },
  {
    "id": 5,
    "name": "Сумочка длинная #05",
    "price": "100",
    "photo": "./media/compositions/oldFruits/IMG_20250811_163445_820.jpg",
    "photo2": "./media/compositions/oldFruits/IMG_20250811_163448_068.jpg",
    "description": "Цветы: хризантемы, зелень <br><br> Зефир с начинками, макаронс, Рафаэлло, орехи в шоколаде, манго, курага, инжир, маракуйя, миндаль <br><br> Заказ 3-5 дней "
  },
  {
    "id": 6,
    "name": "Сумочка длинная #06",
    "price": "105",
    "photo": "./media/compositions/oldFruits/IMG_20250811_163500_639.jpg",
    "photo2": "./media/compositions/oldFruits/IMG_20250811_163503_230.jpg",
    "description": "Курага, кумкват, кизил, инжир, финики, манго, маракуйя, макаронс, орех макадами, миндаль, орехи в белом шоколаде, манго кубик, ананас, груша, хурма целая, хурма кусочки. <br><br> Заказ 3-5 дней "
  },
  {
    "id": 7,
    "name": "Сумочка длинная #07",
    "price": "110",
    "photo": "./media/compositions/oldFruits/IMG_20250811_163512_199.jpg",
    "photo2": "./media/compositions/oldFruits/IMG_20250811_163514_812.jpg",
    "description": "Цветы: хризантемы, зелень <br><br> Курага, кумкват, инжир, финики, хурма кусочки, груша вяленая, хурма целая, манго, маракуйя, ананас, рахат лукум, орех в белом шоколаде <br><br> Заказ 3-5 дней "
  }
];

const Premium = [
    {
    "id": 1,
    "name": "Букет #01",
    "price": "305",
    "photo": "./media/compositions/premium/IMG_20250811_163732_860.jpg",
    "photo2": "./media/compositions/premium/IMG_20250811_163732_956.jpg",
    "description": "Цветы: эустома, хризантема, зелень. <br><br>Сухофрукты: инжир, курага, кумкват, ананас, кизил, манго, финики, хурма целая. <br><br>Орехи: пекан, макадами, миндаль, фундук, фундук в белом шоколаде. <br><br> Диаметр съедобной части около 45см <br>Цена без учёта напитка. <br><br>Заказ 5-7 дней"
  },
  {
    "id": 2,
    "name": "Букет #02",
    "price": "290",
    "photo": "./media/compositions/premium/IMG_20250811_163701_453.jpg",
    "photo2": "./media/compositions/premium/IMG_20250811_163728_975.jpg",
    "description": "Сухофрукты: мед 3шт, манго, курага, кумкват, финик, инжир двух видов, вяленая груша (по сезону), маракуйа, хурма вяленая целая, персик, кубики манго. <br><br>Орехи: пекан, макадами, фундук, миндаль. <br><br>Размер сьедобной части около 40-45см<br><br>Заказ 5-7 дней"
  },
  {
    "id": 3,
    "name": "Букет #03",
    "price": "285",
    "photo": "./media/compositions/premium/IMG_20250811_163733_104.jpg",
    "photo2": "./media/compositions/premium/IMG_20250811_163733_380.jpg",
    "description": "Сухофрукты: Манго двух видов, курага, кумкват, инжир, финики обычный и королевский, ананас двух видов, хурма целая, кизил. <br><br>Орехи: пекан, макадами, миндаль, фундук в белом шоколаде.<br><br>Размеры сьедобной части до 40см <br><br>Заказ 5-7 дней"
  },
  {
    "id": 4,
    "name": "Букет #04",
    "price": "250",
    "photo": "./media/compositions/premium/IMG_20250811_163733_123.jpg",
    "photo2": "./media/compositions/premium/IMG_20250811_163733_553.jpg",
    "description": "Цветы: эустома, эвкалипт <br><br>Сухофрукты: Хурма целая, манго, ананас, курага, кумкват, инжир, палочки корицы, финики. <br><br>Орехи: пекан, макадами, фундук, фундук в белом шоколаде, миндаль.<br><br>Диаметр съедобной части около 40см <br><br>Заказ 5-7 дней "
  },
  {
    "id": 5,
    "name": "Букет #05",
    "price": "300",
    "photo": "./media/compositions/premium/IMG_20250811_163733_603.jpg",
    "photo2": "./media/compositions/premium/IMG_20250811_163732_968.jpg",
    "description": "Цветы: роза кустовая, хризантема, зелень <br><br>Сухофрукты: инжир, кумкват, курага, манго, хурма целая, персик, кубики манго.<br><br> Орехи: пекан, макадами, миндаль, фундук, фундук в белом шоколаде. <br><br>Диаметр съедобной части около 45см <br><br>Заказ 5-7 дней"
  },
  {
    "id": 6,
    "name": "Букет #06",
    "price": "165",
    "photo": "./media/compositions/premium/IMG_20250811_163728_557.jpg",
    "photo2": "./media/compositions/premium/IMG_20250811_163728_769.jpg",
    "description": "Цветы: роза кустовая, гипсофил, зелень <br><br> Сухофрукты: манго, инжир, финики, курага, кумкват, фундук в белом шоколаде, Рафаэлло. <br><br> Диаметр съедобной части около 30см <br><br> Заказ 5-7 дней "
  }
];

const Sea = [
  {
    "id": 1,
    "name": "Ящик дерево #01",
    "price": "250",
    "photo": "./media/compositions/sea/01_01.jpg",
    "photo2": "./media/compositions/sea/01_02.jpg",
    "description": "Раки 1300-1500гр., креветки 500гр., лайм/лимон, зелень.<br><br> Ящик дерево <br><br> Заказ 5-7дней "
  },
  {
    "id": 2,
    "name": "Букет #02",
    "price": "175",
    "photo": "./media/compositions/mans/IMG_20250811_170439_976.jpg",
    "description": "Раки 1кг, креветки 0.5кг лимон/лайм, зелень. <br><br> Цвет упаковки можно менять под запрос <br><br> Заказ 5-7дней "
  },
  {
    "id": 3,
    "name": "Букет #03",
    "price": "130",
    "photo": "./media/compositions/sea/03_01.jpg",
    "photo2": "./media/compositions/sea/03_02.jpg",
    "description": "Раки 1 кг, лимон <br><br> Заказ 5-7дней "
  },
];

function displayProducts(products, containerId) {
    const container = document.getElementById(containerId);

    products.forEach(product => {
        const card = document.createElement('div');
        card.className = 'swiper mySwiper';

        // собираем все фото в массив
        const photos = [product.photo, product.photo2, product.photo3].filter(p => p && p.trim() !== "");

        // формируем слайды динамически
        let slidesHTML = photos.map(p => `
            <div class="swiper-slide"><img src="${p}" alt="" class="product-img"></div>
        `).join("");

        // Пагинация только если есть больше одного слайда
        card.innerHTML = `
            <div class="swiper-wrapper">
                ${slidesHTML}
            </div>
            <div class="pagination-container">
             ${photos.length > 1 ? '<div class="swiper-pagination"></div>' : ""}
            </div>
            <p class="name">${product.name}</p>
            <span class="price">от ${product.price} руб.</span>
            <details class="description"> 
            <summary class="price">Состав:<br></summary>
            ${product.description}<br><br><a href="https://t.me/Ilonapodterebkova" target="_blank" class="btn buy-link">Заказать</a></details>
        `;

         container.appendChild(card);

        // инициализация Swiper только если есть несколько фото
        if (photos.length > 1) {
            new Swiper(card, {
                loop: true,
                slidesPerView: 1,
                spaceBetween: 30,
                pagination: {
                    el: card.querySelector('.swiper-pagination'),
                    clickable: true
                },
                autoplay: {
                    delay: 5000
                },
                keyboard: {
                    enabled: true,
                    onlyInViewport: false
                },
                simulateTouch: true,
                grabCursor: true
            });
        }
    });
}

const currentUrl = window.location.pathname;

if (currentUrl.endsWith("index.html") || currentUrl.endsWith("/BuketBerry/") || currentUrl.endsWith === "/") {
  displayProducts(mans.slice(0,4), 'manItems');
  displayProducts(womans.slice(0, 4), 'womanItems');
  displayProducts(child.slice(0, 4), 'babyItems');
  displayProducts(Fruits.slice(0, 4), 'oldFruitsItems');
  displayProducts(Premium.slice(0, 4), 'premiumItems');
  displayProducts(Sea.slice(0, 4), 'seaItems');
} else{
  displayProducts(mans, 'manItems');
  displayProducts(womans, 'womanItems');
  displayProducts(child, 'babyItems');
  displayProducts(Fruits, 'oldFruitsItems');
  displayProducts(Premium, 'premiumItems');
  displayProducts(Sea, 'seaItems');
}
